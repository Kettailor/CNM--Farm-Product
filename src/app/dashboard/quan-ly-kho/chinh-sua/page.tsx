...
import WarehouseEditClient from "./warehouse-edit-client";

export default async function WarehouseEditPage() {
  const ownerId = layOwnerIdTuServerCookie();
  if (!ownerId) redirect("/login?next=/dashboard/quan-ly-kho/chinh-sua");
  ...
  return (
    <DashboardShell farmName={overview.farmName} activePath="/dashboard/quan-ly-kho">
      <WarehouseEditClient farmName={overview.farmName} items={items} zones={zones} />
    </DashboardShell>
  );
}
