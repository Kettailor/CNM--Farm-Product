...
const normalizeText = (value: unknown) => String(value ?? "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
const typeInfoFromRaw = (raw: string) => {
  const value = normalizeText(raw);
  if (value.includes("cropping") || value.includes("trong trot") || value.includes("cay trong")) return { slug: "trong-trot", label: "Trồng trọt" };
  if (value.includes("pasture") || value.includes("dong co") || value.includes("chan tha")) return { slug: "dong-co", label: "Đồng cỏ" };
  if (value.includes("storage") || value.includes("kho luong thuc") || value.includes("luong thuc") || value.includes("kho")) return { slug: "kho-luong-thuc", label: "Kho lương thực" };
  if (value.includes("parking") || value.includes("bai do xe") || value.includes("phuong tien") || value.includes("vehicle")) return { slug: "bai-do-xe", label: "Bãi đỗ xe" };
  if (value.includes("chan nuoi") || value.includes("vat nuoi")) return { slug: "chan-nuoi", label: "Chăn nuôi" };
  return { slug: "trong-trot", label: "Trồng trọt" };
};
const statusLabelFromRaw = (raw: string) => { const value = normalizeText(raw); if (value.includes("cancel") || value.includes("huy")) return "Đã hủy"; if (value.includes("inactive") || value.includes("ngung")) return "Ngừng hoạt động"; if (value.includes("draft") || value.includes("planned") || value.includes("lap")) return "Dự kiến"; if (value.includes("maintenance")) return "Bảo trì"; if (value.includes("completed")) return "Hoàn thành"; return "Đang hoạt động"; };
...
export async function getZoneList(ownerId: string): Promise<{ farmName: string; zones: ZoneListItem[]; filters: ZoneTypeFilter[]; location: FarmLocation | null }> {
  ...
  const rs = await db.query(
    `select
        ...
        coalesce(kv.hinh_hoc_geojson->'metadata'->>'kind', nullif(loai.ten::text, ''), kv.mo_ta::text, 'cropping') as raw_type,
  ...
  const zones = rs.rows.map((row) => {
    ...
    const typeInfo = typeInfoFromRaw(String(row.raw_type ?? ""));
